...
public sealed class BendCheckService(RuleService ruleService, IStepAnalyzer stepAnalyzer)
{
        ...
        CancellationToken ct)
    {
        ...
        if (rule is null)
        {
            findings.Add(new Finding("FAIL", "RULE_NOT_FOUND", "Keine passende Regelzeile für Material/Dicke/V gefunden."));
        }
        else
        {
            if (rule.MinSchenkelMm is null)
                findings.Add(new Finding("WARN", "MIN_FLANGE_EMPTY", "Schenkelmaß minimal ist in der Regelzeile leer."));
            else
                findings.Add(new Finding("OK", "MIN_FLANGE_RULE", $"Schenkelmaß minimal laut Tabelle: {rule.MinSchenkelMm:0.##} mm (Messung kommt mit STEP-Engine)."));

            findings.Add(new Finding("OK", "TOOL_DEFAULT", "Standardwerkzeug angenommen: 90° Matrize, 88° Stempel, Spitzenradius ~1mm."));
            findings.Add(new Finding("INFO", "AUTO_SUGGEST", "Auto-Verbesserung V1: Vorschläge textlich; echte STEP-Geometrieänderungen folgen später."));
        }

        return new AnalysisResult(stepPath, rulesXlsxPath, material, thicknessMm, rule, findings);
    }
}
